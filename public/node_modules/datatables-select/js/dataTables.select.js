...
(function (factory) {
    ...
    DataTable.select.init = function (dt) {
        ...
        var selectAllClassName = 'select-all';
        ...
        ctx._select.selectAllClassName = selectAllClassName;
    ...
    function disableMouseSelection(dt) {
        ...
        if (ctx._select.style !== 'single') {
            $(dt.table().header())
                .off('click.dtSelectall', 'th.select-checkbox');
        }
    ...
    function enableMouseSelection(dt) {
        ...
        if (ctx._select.style !== 'single') {
            $(dt.table().header())
                .on('click.dtSelectall', 'th.select-checkbox', function (e) {
                    var api = new DataTable.Api(ctx);
                    if ($(this).hasClass(ctx._select.className)) {
                        api.rows().deselect();
                    } else {
                        api.rows().select();
                    }
    ...
    function init(ctx) {
        ...
        if (ctx._select.style !== 'single') {
            ctx.aoHeaderCallback.push({
                fn: function (thead) {
                    $(thead).find('th.select-checkbox').addClass(ctx._select.selectAllClassName);
                }
            });
        }
    ...
    function changeHeader(ctx, api) {
        var selectedAll = false;

        var totalCount = api.rows().count();
        var selectedCount = api.rows({ selected: true }).count();
        if (totalCount) {
            selectedAll = selectedCount === totalCount;
        }

        var $checkboxTh = $(ctx.aoHeader[0].nTr).find('th.select-checkbox');
        if (selectedAll) {
            $checkboxTh.addClass(ctx._select.className);
        } else {
            $checkboxTh.removeClass(ctx._select.className);
        }
    }
